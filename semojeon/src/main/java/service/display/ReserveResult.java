...
public class ReserveResult implements CommandProcess {
	...
	public String requestPro(HttpServletRequest request, HttpServletResponse response) {
		...
		if (request.getParameter("cnt")!=null && display.getFee() != 0) {
			cnt = Integer.parseInt(request.getParameter("cnt"));
		}
		if (request.getParameter("cnt_adult")!=null && display.getFee_adult() != 0) {
			cnt_adult = Integer.parseInt(request.getParameter("cnt_adult"));
		}
		
		if (request.getParameter("cnt_teen")!=null && display.getFee_teen() != 0) {
			cnt_teen = Integer.parseInt(request.getParameter("cnt_teen"));			
		}
		
		if (request.getParameter("cnt_child") != null && display.getFee_child() != 0) {
