...
<% 
    ...
    String id = "";
    int buy_price=0;
    if (session.getAttribute("id") == null){ // login 안했으면
       id = "not";
...
%>
     ...
     <table border="0" width="650" cellpadding="5" cellspacing="5" align="center"> 
       ...
       <tr><td>정가 : <span style="text-decoration:line-through; ">
       	<fmt:formatNumber value="<%=book.getBook_price() %>"/>원</span><br>
            <%buy_price = (int)(book.getBook_price()*((double)(100-book.getDiscount_rate())/100)) ;%>
            판매가 : <b><fmt:formatNumber value="<%=(int)(book.getBook_price()*((double)(100-book.getDiscount_rate())/100))%>"/>원</b></td> 
       <tr><td>수량 : <input type="text" size="5" name="buy_count" value="1"> 개
              <%
                 if(id.equals("not")){ // 로그인 안함 경우
                	 if(book.getBook_count()==0){
              %>
                		 <b>일시품절</b>
              <%    }    %>                
             <%}else{  if(book.getBook_count()<=0){  // 로그인 %>
                         <b>일시품절</b>                           
             <%      }else{    %>	 
    ...
    <%      }
            ...
            <input type="button" value="목록으로" onclick="location.href='shopMain.jsp?pgm=list.jsp?book_kind=<%=book_kind%>'">
